...
boxplot6.2 = function(df,name){
   for (distance in c("euklid","winkel","winkellog")){
    ...
    jpeg(filename = filename,units="in",width=5,height=5,res=300)
    myplot <- ggplot(df.sub, aes(y=query.berfore.ref..qbr., x= Video, color = speed.f)) + geom_boxplot() + labs(title = paste0("Distanz : ",distance))
    print(myplot)
    dev.off()
   }
}
...
histogramm.bins.plot_1  = function(df,name){
   for (distance in c("euklid","winkel","winkellog")){
    ...
    df.sub.h <- subset(df.sub.t, select = -c(Video, IP, Distancemeasure, total_distance, len_of_signal, mean_distance, median_distance, zero.overhead.counter, ref.before.query..rbq., query.berfore.ref..qbr., max.rbq, max.qbr, max.distance, min.distance, Block, speed, PB))
